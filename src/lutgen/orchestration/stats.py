...
def compute_stats_batch(images) -> list:
    """``compute_stats`` over several images, serial. (Threading the BLAS-heavy stats — np.cov /
    matmul — can intermittently segfault via OpenBLAS even with the thread cap, so we keep it
    serial for stability. The parallel image DECODE in ingest is BLAS-free and safe.)"""
